...
jQuery(document).ready(function ($) {
	...
	$(".divScaleSelect > div").mouseup(function (event) {
		...
		$("#imgScale").addClass("imgScale"+x.replace(".","_"));
		$(".divScaleSelect").hide();
	...
	$(".btnOK").mouseup(function (event) {
		...
		Start = parseFloat($(".inputMin").val());
		End = parseFloat($(".inputMax").val());

		var Check = $(".inputMin").val().toString().split('.');
		if (Check.length > 1) {
			if (Check[1].length > 3) {
				alert("最小值最多到小數點第三位");
				return;
		...
		Check = $(".inputMax").val().toString().split('.');
		if (Check.length > 1) {
			if (Check[1].length > 3) {
				alert("最大值最多到小數點第三位");
				return;
		...
		scale = parseFloat($("#imgScale").attr("num"));
